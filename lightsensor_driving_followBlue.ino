...
void loop() {
    ...
    currentMillis = millis();
        ...
        if (red_output == 0) {
            ...
            if (analogRead(sensor) < bbThresh) {
                // BLACK
                surfaceColor = 1;
                Serial.println("I'm on black!");
            } else {
                // BLUE
                surfaceColor = 2;
                Serial.println("I'm on blue!");
            }
        } else {
            if (analogRead(sensor) < ryThresh) {
                // RED
                surfaceColor = 3;
                Serial.println("I'm on red!");
            } else {
                // YELLOW
                surfaceColor = 4;
                Serial.println("I'm on yellow!");
            }
        }
        // reset timer counter
    ...
    } else if (lostCount < 1000) { 
        // check right
        drive_mode = 3;
        lostCount = lostCount+1;
        //Serial.println("going right");
    } else if (lostCount < 2000) {
        // check left
        drive_mode = 4;
        lostCount = lostCount+1;
