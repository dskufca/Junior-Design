...
void setup() {
  ...
  /*attachInterrupt(digitalPinToInterrupt(switch_one), decBlueDelay, RISING);*/
  attachInterrupt(digitalPinToInterrupt(switch_one), blueDelay, CHANGE);
  /*attachInterrupt(digitalPinToInterrupt(switch_two), redLedOff, RISING);*/
...
void loop() {
    ...
    switch(sys_state) {
        ...
        case 3: {
            ...
            currentMillis = millis();
            if (currentMillis - previousMillis >= 500 and fade_status == HIGH) {
                brightness = brightness/1.59;
                previousMillis = currentMillis;
            }
            // if brightness too low, switch to blink
            if (currentMillis - previousBlinkMillis >= 6000) {
                fade_status = LOW;
                brightness = 255;
                runLed_state = HIGH;
            ...
            // BLINK GREEN LED
            currentMillis = millis();
            if (currentMillis - previousMillis >= 500 and fade_status == LOW) {
                if (runLed_state == LOW) {
                    runLed_state = HIGH;
                } else {
                    runLed_state = LOW;
